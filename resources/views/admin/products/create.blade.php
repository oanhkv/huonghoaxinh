    ...
    <div class="card admin-form-card shadow-sm">
        ...
        <div class="p-4 p-lg-5">
            <form action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="admin-form-panel mb-4">
                    @php
                        $oldColors = (array) old('colors', []);
                        $oldMaterials = (array) old('materials', []);
                    @endphp
                    <div class="row g-4">
                        <div class="col-lg-6">
                            ...
                            <div class="color-swatch-grid">
                                @foreach($colorOptions as $name => $swatch)
                                    <label class="color-swatch">
                                        <input type="checkbox" name="colors[]" value="{{ $name }}" {{ in_array($name, $oldColors, true) ? 'checked' : '' }}>
                        ...
                        <div class="col-lg-6">
                            ...
                            <div class="chip-grid">
                                @foreach($materialOptions as $material)
                                    <label class="chip">
                                        <input type="checkbox" name="materials[]" value="{{ $material }}" {{ in_array($material, $oldMaterials, true) ? 'checked' : '' }}>
                                        <span>{{ $material }}</span>
                                    </label>
                                @endforeach
                            </div>
